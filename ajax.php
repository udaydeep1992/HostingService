...
$daemonConfigFile = "/var/HOSTINKEY/data/hostinkey.conf";
...
//////////////////////////////
function readInfo() {
	$d = file_get_contents("/var/HOSTINKEY/services/data/getinfo");
	return json_decode($d, true);
}
function readPeerInfo() {
	$d = file_get_contents("/var/HOSTINKEY/services/data/getpeerinfo");
	return json_decode($d, true);
	
}
function readMasterNodeListFull() {
	$d = file_get_contents("/var/HOSTINKEY/services/data/masternode_list_full");
	return json_decode($d, true);
	
}
function readMasterNodeListRank() {
	$d = file_get_contents("/var/HOSTINKEY/services/data/masternode_list_rank");
	return json_decode($d, true);
	
}
function readMasterNodeStatus() {
	$d = file_get_contents("/var/HOSTINKEY/services/data/masternode_status");
	return json_decode($d, true);
...
function getPayoutData($walletAddr) {
	$d = file_get_contents("https://hostinkey.blockchain-explorers.info/ext/getaddress/".$walletAddr);
	return json_decode($d, true);
}
//////////////////////////////
//		MASTERNODE INFO
...
function Info()
{
	...
	if (@fsockopen("127.0.0.1", 3310, $errno, $errstr, 1)) $arr['status'] = true; else $arr['status'] = false;
	...
	if($mnStatus)
	{
		...
		$arr['masternodeWalletBalance'] = file_get_contents("http://explorer.hostinkey.org/ext/getbalance/".$arr['masternodePayoutWallet']);
	}
...
function restartDaemon()
{
	...
	if($latestVersion != "" && $latestVersion != md5_file("/var/HOSTINKEY/hostinkeyd")) {
		set_time_limit(1200);
...
function resetServer()
{
	...
	sleep(10);
	print_r(exec('sudo rm /var/HOSTINKEY/hostinkeyd'));
	die();
}
